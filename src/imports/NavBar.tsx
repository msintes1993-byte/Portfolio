import svgPaths from "./svg-dwa9xydc08";
...
function LangSwitch({ className, lang = "esp" }: LangSwitchProps) {
  const isEn = lang === "en";
  return (
    <button className={className || "content-stretch flex items-center justify-center p-[10px] relative"}>
      <p className="font-['Michroma:Regular',sans-serif] leading-[0] not-italic relative shrink-0 text-[#0f0f0f] text-[14px] text-left tracking-[1.68px]">
        <span className={`leading-[normal] ${isEn ? "" : "text-[#ff6b2e]"}`}>{isEn ? "ES  // " : "["}</span>
        <span className={`leading-[normal] ${isEn ? "text-[#ff6b2e]" : ""}`}>{isEn ? "[" : "ES"}</span>
        <span className={`leading-[normal] ${isEn ? "" : "text-[#ff6b2e]"}`}>{isEn ? "EN" : "]"}</span>
        <span className={`leading-[normal] ${isEn ? "text-[#ff6b2e]" : ""}`}>{isEn ? "]" : " //  EN"}</span>
      </p>
    </button>
  );
...
function Links({ className, type = "defalut" }: LinksProps) {
  const isHover = type === "hover";
  return (
    <div className={className || `content-stretch flex flex-col items-center justify-center p-[10px] relative ${isHover ? "" : "min-w-[86px]"}`}>
      <div className={`content-stretch flex items-center justify-center p-[4px] relative shrink-0 ${isHover ? "font-['Satoshi:Regular',sans-serif] gap-[10px] leading-[24px] not-italic text-[14px]" : ""}`} data-name="content">
        <p className={`relative shrink-0 text-[#1a1a1a] ${isHover ? "" : "font-['Satoshi:Regular',sans-serif] leading-[24px] not-italic text-[14px]"}`}>{isHover ? "[" : "LINK"}</p>
        {isHover && (
          <>
            <p className="relative shrink-0 text-[#c43200]">LINK</p>
            <p className="relative shrink-0 text-[#1a1a1a]">]</p>
          </>
        )}
      </div>
    </div>
...
export default function NavBar({ className }: { className?: string }) {
  return (
    <div className={className || "bg-[#fbf9f5] content-stretch flex flex-col items-center justify-center pb-[32px] pt-[24px] px-[24px] relative"} data-name="nav-bar">
      ...
      <div className="content-stretch flex gap-[48px] items-center justify-center px-[24px] py-[16px] relative shrink-0" data-name="content">
        <div className="content-stretch flex flex-col gap-[10px] items-center justify-center leading-[normal] relative shrink-0" data-name="logo">
        ...
        <div className="content-stretch flex flex-col gap-[2px] items-center justify-center relative shrink-0" data-name="links">
          <Links className="content-stretch flex flex-col items-center justify-center min-w-[86px] p-[10px] relative shrink-0" />
        ...
        <LangSwitch className="content-stretch cursor-pointer flex items-center justify-center p-[10px] relative shrink-0" />
